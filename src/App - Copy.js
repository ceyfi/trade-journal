...
// ─── CONFIGURATION ────────────────────────────────────────────────────────────
// Replace these two lines with your real Supabase credentials
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .app { max-width: 520px; margin: 0 auto; padding: 0 0 80px; min-height: 100vh; position: relative; }

  /* Header */
  .header { padding: 28px 20px 0; display: flex; align-items: center; justify-content: space-between; margin-bottom: 28px; }
  .logo { font-family: 'Space Mono', monospace; font-size: 13px; color: var(--accent); letter-spacing: 0.12em; }
  ...
  .badge { font-size: 10px; padding: 3px 8px; border-radius: 20px; font-family: 'Space Mono', monospace; letter-spacing: 0.04em; }
  ...
  .btn-danger { background: var(--red-dim); border: 1px solid #ff406030; color: var(--red); }
  ...
  .setup-notice { background: var(--amber-dim); border: 1px solid #f5a62330; border-radius: 12px; padding: 14px 16px; margin: 0 16px 16px; font-size: 13px; color: var(--amber); line-height: 1.6; }
  .setup-notice code { font-family: 'Space Mono', monospace; font-size: 11px; background: #f5a62320; padding: 2px 6px; border-radius: 4px; }
`;
...
export default function TradeJournal() {
  ...
  useEffect(() => { if (isConfigured) loadTrades(); else setLoading(false); }, []);
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {screen === "log" && (
          <LogTrade onSave={async (t) => { await loadTrades(); setScreen("dashboard"); }} onBack={() => setScreen("dashboard")} />
        )}
        {screen === "detail" && selected && (
          <TradeDetail trade={selected} onBack={() => { setSelected(null); setScreen("dashboard"); }} onClose={async (data) => { await loadTrades(); setSelected(null); setScreen("dashboard"); }} />
        )}
        {screen === "review" && (
          <Review trades={closedTrades} adherenceRate={adherenceRate} winRate={winRate} />
        )}
      </div>
    </>
  );
...
// ─── DASHBOARD ───────────────────────────────────────────────────────────────
function Dashboard({ trades, openTrades, closedTrades, adherenceRate, winRate, loading, isConfigured, onNew, onSelect }) {
  return (
    <>
      {!isConfigured && (
        <div className="setup-notice">
          ⚙️ Add your Supabase credentials to the top of this file, then create a <code>trades</code> table. See console for setup SQL.
        </div>
      )}
          ...
          <>
            ...
            <div className="detail-grid">
              ...
              <div className="detail-field">
                ...
                <div className={`detail-value ${trade.pnl >= 0 ? "green" : "red"}`} style={{ color: trade.pnl >= 0 ? "var(--green)" : "var(--red)", fontWeight: 700 }}>{fmt(trade.pnl)}</div>
              </div>
            </div>
            <div className="detail-field">
...
// ─── REVIEW ───────────────────────────────────────────────────────────────────
function Review({ trades, adherenceRate, winRate }) {
